...
class CCalendar
{
	...
	public static function GetDavCalendarEventsList($calendarId, $arFilter = [])
	{
		...
		foreach ($arEvents as $event)
		{
			...
			$event['FIO'] = CCalendarEvent::ParseRRULE($event['FIO']);
			$event['PHONE'] = CCalendarEvent::ParseRRULE($event['PHONE']);
			$result[] = $event;
		}
	...
	public static function SaveEventEx($params = [])
	{
		...
		if (!$bNew)
		{
			...
			$arFields['PHONE'] = $curEvent['PHONE'];
			$arFields['FIO'] = $curEvent['FIO'];
			...
			)
			{
				...
				foreach($events as $ev)
				{
					if ($ev['ID'] !== $curEvent['ID'])
					{
						...
						$newParams['arFields']['RRULE'] = CCalendarEvent::ParseRRULE($ev['RRULE']);
						$newParams['arFields']['FIO'] = CCalendarEvent::ParseRRULE($ev['FIO']);
						$newParams['arFields']['PHONE'] = CCalendarEvent::ParseRRULE($ev['PHONE']);
						$newParams['recursionEditMode'] = 'skip';
