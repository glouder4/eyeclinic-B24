...
class CCalendarEvent
{
	...
	public static function Edit($params = [])
	{
		...
		if (self::CheckFields($entryFields, $currentEvent, $userId))
		{
			...
			if (!empty($dbFields['FIO']))
			{
				$dbFields['FIO'] = Emoji::encode($dbFields['FIO']);
			}
			if (!empty($dbFields['PHONE']))
			{
				$dbFields['PHONE'] = Emoji::encode($dbFields['PHONE']);
			}
					...
					)
					{
						...
						)
						{
							...
							CCalendarNotify::Send([
								...
								'fio' => $entryFields['FIO'] ?? null,
								'phone' => $entryFields['PHONE'] ?? null,
	...
	private static function getListOrm($params = [])
	{
		...
		while ($event = $queryResult->fetch())
		{
			...
			if (!empty($event['FIO']))
			{
				$event['FIO'] = Emoji::decode($event['FIO']);
			}
			if (!empty($event['PHONE']))
			{
				$event['PHONE'] = Emoji::decode($event['PHONE']);
			}
	...
	private static function getListOld($params = [])
	{
		...
		if (is_array($arFilter))
		{
			...
			for ($i = 0, $l = count($filter_keys); $i<$l; $i++)
			{
				...
				elseif ($n === 'FIO')
				{
					$arSqlSearch[] = "CE.FIO='".$DB->ForSql($val)."'";
				}
				elseif ($n === 'PHONE')
				{
					$arSqlSearch[] = "CE.PHONE='".$DB->ForSql($val)."'";
				}
	...
	private static function GetFields()
	{
		...
		if (!count(self::$fields))
		{
			...
			self::$fields = array(
				...
				"FIO" => Array("FIELD_NAME" => "CE.FIO", "FIELD_TYPE" => "string"),
				"PHONE" => Array("FIELD_NAME" => "CE.PHONE", "FIELD_TYPE" => "string"),
